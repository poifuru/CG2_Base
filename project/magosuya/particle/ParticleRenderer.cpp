	...
	case ParticleType::Standard:
		{
			...
			for(uint32_t i = 0; i < data_->kMaxParticleNum; ++i) {
				...
				data_->materialData_[i].uvTransform = Math::MakeIdentity4x4();
			}
			...
			data_->psoDesc.VS_ID = ShaderManager::GetInstance()->CompileAndCacheShader(L"Resources/shader/Particle.VS.hlsl", L"vs_6_0");
			data_->psoDesc.PS_ID = ShaderManager::GetInstance()->CompileAndCacheShader(L"Resources/shader/Particle.PS.hlsl", L"ps_6_0");
