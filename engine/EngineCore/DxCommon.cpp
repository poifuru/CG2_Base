...
void DxCommon::Initialize () {
	...
	vertexShaderBlob = CompilerShader (L"resources/shader/Object3d.VS.hlsl",
														L"vs_6_0", dxcUtils.Get (), dxcCompiler.Get (), includeHandler.Get (), logStream);
	assert (vertexShaderBlob != nullptr);

	pixelShaderBlob = CompilerShader (L"resources/shader/Object3d.PS.hlsl",
...
#pragma endregion
	...
	textureResource0 = CreateTextureResource (device.Get (), metadata0);
	intermediateResource0 = UploadTextureData (textureResource0, mipImages[0], device.Get (), commandList);
	...
	textureSrvHandleGPU;
	textureSrvHandleCPU = GetCPUDescriptorHandle (srvDescriptorHeap.Get (), descriptorSizeSRV, 1);
	textureSrvHandleGPU = GetGPUDescriptorHandle (srvDescriptorHeap.Get (), descriptorSizeSRV, 1);
	//SRVの生成
	device->CreateShaderResourceView (textureResource0.Get (), &srvDescSphere, textureSrvHandleCPU);
